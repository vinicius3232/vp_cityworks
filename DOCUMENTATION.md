...
- **Painel/Multímetro** (`panel`) — **quadro de disjuntores** (breaker branco c/ terminais + janela OFF, caixa esverdeada) + **multímetro DT-830B (foto real `html/img/multimetro.png`)** com LCD ao vivo sobreposto (mascara o "0.00" impresso; verde normal ~220V, vermelho no defeituoso) + **ponteira vermelha como cursor** (agulha metálica que toca o disjuntor; some na fase de reparo). Dinâmica: **segurar pra medir** (leitura flutua ~0,65s e estabiliza) + **beep de continuidade** (tom grave intermitente no defeito, agudo no normal); errar **não falha na hora** — dá tentativas (`maxWrong`, padrão 2; estourou = choque) com shake no disjuntor. Achar o de voltagem **anormal** → remover 4 parafusos → trocar switch → reapertar. Por padrão usa a **foto real do disjuntor** (`html/img/disjuntor.png`, DEP3, feita pelo dono) via `settings.image`. Para voltar ao desenho CSS, remova o campo `image` em `minigames.panel`.
