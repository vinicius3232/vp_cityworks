...
Reescritos do zero. Visual CSS, **áudio sintetizado (WebAudio)**, sem assets proprietários. Resultado unificado: `POST minigameResult { success }`. **Juice in-game:** alvos de minigame **faíscam (ptfx elétrico)** quando quebrados (visíveis até ~35m) e dão um **estouro de faíscas** ao serem consertados (`burstFx`).

- **Solda** (`welding`) — **traçar a trinca** numa **chapa de metal escura** (combina com a UI dark/neon): segurar e arrastar o maçarico sobre a linha em zigue-zague (projeção no caminho, avanço monotônico) enquanto o metal derretido preenche atrás com brilho + faíscas; sair da linha esfria (penalidade) → perde tentativa; timer. Settings: `wireCount` (nº de costuras), `maxFails`, `time`.
- **Painel/Voltímetro** (`panel`) — **quadro de disjuntores estilizados** (caixa escura + 4 parafusos + alavanca azul, board escuro) + **voltímetro DESENHADO** (estilo base: device escuro, display neon verde/vermelho, **luzes** indicadoras, knob e **3 cabos brilhantes** verde/vermelho/amarelo) + **ponteira vermelha como cursor**. Hook opcional `settings.image` ainda permite foto no disjuntor. (agulha metálica que toca o disjuntor; some na fase de reparo). Dinâmica: **segurar pra medir** (leitura flutua ~0,65s e estabiliza) + **beep de continuidade** (tom grave intermitente no defeito, agudo no normal); errar **não falha na hora** — dá tentativas (`maxWrong`, padrão 2; estourou = choque) com shake no disjuntor. Achar o de voltagem **anormal** → remover 4 parafusos → trocar switch → reapertar. Por padrão usa a **foto real do disjuntor** (`html/img/disjuntor.png`, DEP3, feita pelo dono) via `settings.image`. Para voltar ao desenho CSS, remova o campo `image` em `minigames.panel`.
