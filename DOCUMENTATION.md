...
- **Painel/Multímetro** (`panel`) — **quadro de disjuntores** (breaker branco c/ terminais + janela OFF, caixa esverdeada) + **multímetro DT-830B desenhado** (LCD ao vivo: verde normal ~220V, vermelho no defeituoso) + **ponteira vermelha como cursor** (agulha metálica que toca o disjuntor; some na fase de reparo); passar sobre os disjuntores, achar o de voltagem **anormal** → remover 4 parafusos → trocar switch → reapertar. Opcional: `settings.image = 'nui://.../meu_disjuntor.png'` usa uma **foto** no lugar do desenho do disjuntor (use só imagem com direitos/licença livre).
