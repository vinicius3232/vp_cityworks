...
- **Painel/Multímetro** (`panel`) — **quadro de disjuntores** (breaker branco c/ terminais + janela OFF, caixa esverdeada) + **multímetro DT-830B desenhado** (LCD ao vivo: verde normal ~220V, vermelho no defeituoso) + **ponteira vermelha como cursor** (agulha metálica que toca o disjuntor; some na fase de reparo); passar sobre os disjuntores, achar o de voltagem **anormal** → remover 4 parafusos → trocar switch → reapertar. Por padrão usa a **foto real do disjuntor** (`html/img/disjuntor.png`, DEP3, feita pelo dono) via `settings.image`. Para voltar ao desenho CSS, remova o campo `image` em `minigames.panel`.
