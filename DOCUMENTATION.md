...
- **Painel/Voltímetro** (`panel`) — **quadro de disjuntores** (breaker branco c/ terminais + janela OFF, caixa esverdeada); passar o voltímetro, achar o de voltagem **anormal** → remover 4 parafusos → trocar switch → reapertar. Opcional: `settings.image = 'nui://.../meu_disjuntor.png'` usa uma **foto** no lugar do desenho (use só imagem com direitos/licença livre).
